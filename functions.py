...
#             continue
        ...
        if 'ls' not in p.keys():
            ...
            ls = p['ls']
        ax.plot(np.mean(rnn_activity[trials, :, 0, n], axis=0), alpha=0.3, linewidth=10, color=color, label=name, ls=ls)
        for tr in trials:
#             pass
            ax.plot(rnn_activity[tr, :, 0, n], color=color, linewidth=1, alpha=0.0, ls=ls)
