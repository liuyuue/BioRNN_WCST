...
#     l2_hs = [0, 1e-5, 1e-4, 1e-3]
    l2_hs = [0, 1e-4, 1e-3]
#     optimizers = [torch.optim.Adam, torch.optim.Rprop, torch.optim.Adadelta, torch.optim.Adagrad, torch.optim.AdamW, torch.optim.Adamax, torch.optim.ASGD, torch.optim.RMSprop, torch.optim.SGD]    # Adadelta, Adagrad, Adam, AdamW, Adamax, ASGD, RMSprop, Rprop, SGD
    optimizers = [torch.optim.Adam, torch.optim.Rprop, torch.optim.SGD]    # fewer
    initializations = ['orthogonal', 'kaiming_normal', 'kaiming_uniform', 'constant']
    torch_seeds = [1,10,20,30,40]
#     lrs = [1e-3, 5e-4, 1e-4]
    lrs = [1e-3]

    for optimizer, lr, l2_h, initialization, torch_seed in itertools.product(optimizers, lrs, l2_hs, initializations, torch_seeds):
        print('\n\n\nSTART TRAINING A NEW MODEL...\nl2_h={}, lr={}, optimizer={}, torch_seed={}, initialization={}\n\n\n'
                  .format(l2_h, lr, optimizer, torch_seed, initialization), flush=True)
        ...
        hp['torch_seed'] = torch_seed
        torch_seed = torch_seed; torch.manual_seed(torch_seed);
        random.seed(0)
        np.random.seed(0)
...
#                 customization+=('somanonlinear{}_'.format(hp[key]))
        ...
        save_name = '{}_init={}_l2h={}_torchseed={}_lr={}_optim={}'.format(hp['jobname'], initialization, l2_h, torch_seed, lr, optimizer.__name__)
        hp['save_name'] = save_name
        ...
        optimizer = optimizer     # customize optimizer: Adadelta, Adagrad, Adam, AdamW, Adamax, ASGD, RMSprop, Rprop, SGD
...
#             if torch.isnan(grad_norm)==False:
            ...
            if hp['timeit_print']==True:
                print('***backward pass takes {}s'.format(time.time()-start_backward), flush=True)
...
#                 print('Step {}, SST bias {}'.format(ba, model.rnn.bias[model.rnn.cg_idx['sr_sst']]))
                ...
                print('prev stim/choice/rew mag: {}/{}/{}\n'.format(model.rnn.prev_stim_mag, model.rnn.prev_choice_mag, model.rnn.prev_rew_mag))
...
#                 if True:    # for debugging
                    ...
                    if np.mean(test_perf_list_big[-1:])>=max_perf_test-0.05 and np.mean(test_perf_rule_list_big[-1:])>=max_perf_test-0.05:
                        if curriculum==False:
                            ...
                            if give_prev_stim==True:    # curriculum learning on trial history, progress +1
                                ...
                                print('******** Now not giving previous stimulus ********', flush=True)
                                continue
                            elif give_prev_choice==True and model.rnn.prev_stim_mag==0:    # curriculum learning on trial history, progress +1
                                give_prev_choice = False
                                print('******** Now not giving previous choice ********', flush=True)
                                continue
                            else:
                                if curriculum_t==False and model.rnn.prev_choice_mag==0:    # curriculum learning on trial history completed
        ...
        print('saving model. name={}_{} ... \n'.format(success, save_name))
        saved_file = {'model': model, 'rnn_activity_lastk': rnn_activity_list, 'model_state_dict': model.state_dict(), 'optimizer': optim, 'optim_state_dict': optim.state_dict(), 'hp': hp, 'hp_task': hp_task, 
                    'perf_list':perf_list_big, 'perf_rule_list': perf_rule_list_big, 'loss_list': loss_list_big,
                    'test_perf_list': test_perf_list_big, 'test_perf_rule_list': test_perf_rule_list_big, 'test_loss_list': test_loss_list_big, 'torch_seed': torch_seed}
#         torch.save(saved_file, '/home/yl4317/Documents/two_module_rnn/saved_models/{}_{}'.format(success, save_name[:150]))  
        torch.save(saved_file, '/scratch/yl4317/two_module_rnn/saved_models/{}_{}'.format(success, save_name[:150]))     # save a copy in scratch
