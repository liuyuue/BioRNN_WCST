...
#         noise[:, :, self.pfc_idx] = _noise.unsqueeze(dim=2)    # only apply noise to pfc
        ...
        ## 2. independent noise
#         noise = torch.randn_like(input)*torch.sqrt(torch.tensor(self.decay))
#         noise_last = None
...
#             print('hiddens={}'.format(hiddens))
        ...
        last_states = {'hidden': h_last, 'i_me': i_me_last, 'noise': noise_last}
        record = {'hiddens': hiddens, 'i_mes': i_mes, 'noise': self.network_noise*noise}
